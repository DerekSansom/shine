...
public class MessageHandler {
	...
	@Transactional
	public void persistMessages(Queue<UserNotification> messageQueue) {
...
//TODO: decide what to do here e.g. push notify, send internal message, send email
		...
		if (p != null) {
			...
			case BOARD_POSTED_TO:
				sentPushNotification = sendUserPushNotification(p, C2DM_NEW_POST_ON_BOARD.charAt(0), un.intExtra);
				sendMessage(p, un, sentPushNotification);
				sendBoardPostedToEmail(p, un);
				break;
			case MAIL_RECEIVED:
				sentPushNotification = sendUserPushNotification(p, C2DM_NEW_MSG.charAt(0), null);
	...
	private void sendBoardPostedToEmail(PlayerEntity p, UserNotification un) throws ShineException {
		...
		String message = String.format(BOARD_POSTED_TEXT, board.getName(), createBoardUrl(board.getId()));
	...
	private void sendPostRepliedToEmail(PlayerEntity p, UserNotification un) throws ShineException {
		...
		String message = String.format(POST_REPLIED_TEXT, notice.getTitle(), createPostUrl(notice.getBoardId(), notice.getId()));
		String subject = String.format(getLanguageString("en", "Streetpin post %s replied to"), notice.getTitle());
		sendEmail(p.getForename(), p.getEmail(), subject, message);
	}

	private Object createBoardUrl(int id) {
		String url = ShineProperties.getDomainUrl() + "/here/#boardscreen?id=" + id;

		return url;
	}

	private Object createPostUrl(int boardid, int postid) {
		String url = ShineProperties.getDomainUrl() + "/here/#posts?boardid=" + boardid + "&pid=" + postid;
